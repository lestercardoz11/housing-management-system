...
{
    public partial class PropertySell : System.Web.UI.Page
    {
        int UID;
        char UT;
        int HID;
        ...
        public void Notification(SqlConnection cnn, string notiftype, string notif)
        {
            ...
            using (SqlCommand cmd = new SqlCommand(sql, cnn))
            {
                ...
                {
                    System.Windows.Forms.MessageBox.Show("Sale Ad could not be displayed.");
                }
            }
        }
        ...
        protected void BSell_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                using (SqlConnection cnn = new SqlConnection("Data Source = JARVIS; Initial Catalog = HousingMSdb; User ID = sa; Password = 2411"))
                {
                    ...
                    sql = "IF NOT EXISTS (SELECT * FROM Sell WHERE HID=@HID) INSERT INTO Sell ([HID], [SID], [UID], [Price], [EntryDate]) values(@HID, @SID, @UID, @Price, @EntryDate)";
                    cnn.Open();
                    try
                    {
                        SqlDataAdapter adapter = new SqlDataAdapter();
                        using (SqlCommand cmd = new SqlCommand(sql, cnn))
                        {
                            ...
                            if (cmd.ExecuteNonQuery() == 1)
                            {
                                ...
                                string notif = "Apartment " + lblApartmentNo.Text + " Sale Ad is being displayed.";
